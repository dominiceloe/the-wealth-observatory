# Step 02: Database Schema Creation

## Objective
Create all PostgreSQL database tables with proper relationships, indexes, and constraints.

## Prerequisites
- PostgreSQL 14+ running on port 5433
- Database created: `wealthobservatory`
...
```sql
CREATE DATABASE wealthobservatory;
```

### 2. Create Schema Migration File
...
```bash
psql -h localhost -p 5433 -U your_user -d wealthobservatory -f scripts/schema.sql
```

Or via Node.js script (create `scripts/run-migration.ts`):
```typescript
import { Pool } from 'pg';
...
## Next Step
Proceed to `STEP-03-DATABASE-CONNECTION.md` to set up database connection utilities.
